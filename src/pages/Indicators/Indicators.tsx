...
import { getDomainAutocompleteObjects, getDomainDisplayValue, getIndicatorTypeAutocompleteObjects, getIndicatorTypeDisplayValue, getTablePageSize, handleHttpError, i18n, updateArtifactsCount } from '../../utils';
import { renderDate, Table, TableDataRequest } from '../../components/Table';
import { Loader } from '../../components/Loader';
import { createIndicator, deleteIndicator } from '../../services/pages/indicators';
...
export function Indicators() {
  ...
  const [createData, setCreateData] = useState({ name: '', indicator_type_id: '', domain_id: '' });
  ...
  const submitCreate = () => {
    if (createData.name && createData.indicator_type_id && createData.domain_id) {
      setShowCreateDlg(false);

      createIndicator(createData).then(json => {
  ...
  return (
    <div className={classNames(styles.page, styles.scrollable, { [styles.loaded]: loaded })}>
      {!loaded ? (
        ...
        <>
          <div className={styles.title}>{`${i18n('Показатели')}`}<Button background='blue' onClick={() => { setShowCreateValidation(false); setShowCreateDlg(true); }}>Создать показатель</Button></div>
          <Table
          ...
          <ModalDlg show={showCreateDlg} title={i18n('Создать показатель')} cancelBtnText={i18n('Отменить')} submitBtnText={i18n('Создать')} onClose={() => setShowCreateDlg(false)} dialogClassName={styles.dlg_create} onSubmit={submitCreate}>
            <div className={styles.fields}>
                <FieldTextEditor label={i18n('Название показателя')} isRequired showValidation={showCreateValidation} className='' defaultValue='' valueSubmitted={(v) => setCreateData((prev) => ({...prev, name: v ?? ''}))} />
                ...
                <FieldAutocompleteEditor
                  label={i18n('Тип')}
                  defaultValue=''
                  valueSubmitted={(v) => setCreateData((prev) => ({...prev, indicator_type_id: v ?? ''}))}
