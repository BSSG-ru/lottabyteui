...
###  Архитектура Lottabyte
 ...
 Основное архитектурное решение базируется на том, что все компоненты Lottabyte дислоцированы в Kubernetes. Опционально, компоненты могут быть развернуты без оркестратора контейнеров на физических или виртуальных сервера. Программное обеспечение Lottabyte разработано на базе концепции API-first, для реализации бизнес-логики (API) применяется Java (spring), пользовательский интерфейс разработан с использованием фреймворка React на TS. Подсистема выполнения правил качества реализовано на python.
